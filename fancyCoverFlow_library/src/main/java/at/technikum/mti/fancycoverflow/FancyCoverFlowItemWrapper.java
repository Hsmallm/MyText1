...
class FancyCoverFlowItemWrapper extends ViewGroup {
    ...
    private void remeasureChildren() {
        ...
        if (child != null) {
            ...
            //originalChildHeight:表示整体的高度,即为间距+显示倒影部分+不显示倒影部分
            //imageReflectionRatio：表示倒影显示部分比例
            //reflectionGap：表示间距部分
    ...
    private void createReflectedImages() {
        ...
        //originalScaledownFactor:未显示部分比例
        //倒影未显示部分的高度
        ...
        //invertedBitmapSourceTop：未显示高度-显示高度，注：其值不能小于0
        final int invertedBitmapSourceTop = scaledDownHeight - invertedHeight;
        //matrix：通过matrix使整个图像倒置过来
